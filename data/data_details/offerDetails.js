  ...
  fetch("/data/api/apiStore.json")
    ...
    .then((apiStore) => {
      ...
      if (gameFound) {
        ...
        originalPrice.textContent = `CLP$ ${gameFound.precioOriginal}`;
        finalPrice.textContent = `CLP$ ${gameFound.precioDescuento}`;
...
function giveALike() {
  ...
  fetch("/data/api/apiStore.json")
    .then((response) => response.json())
    .then((apiStore) => {
      const gameFound = apiStore.offerCards.find((game) => game.id === gameId);
      if (gameFound) {
        const gameLikes = document.getElementById("gameLikes");
        if (gameLikes) {
          gameLikes.textContent = gameFound.likes;
  ...
  likesButton.addEventListener("click", function () {
    fetch("/data/api/apiStore.json")
      .then((response) => response.json())
      .then((apiStore) => {
        const gameFound = apiStore.offerCards.find(
          (game) => game.id === gameId
        );
        if (gameFound) {
          if (liked) {
